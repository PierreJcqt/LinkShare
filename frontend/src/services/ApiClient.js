  ...
  return fetch(this.baseUrl + path, {
    ...
    .catch(error => {
      if (error.message === 'Ressource not found') {
        ...
        console.error("Impossible de récupérer les données de l'API");
      }
  ...
  delete(path) {
    return fetch(this.baseUrl + path, {
      ...
      .then(response => {
        ...
        if (!response.ok) {
          console.error('Statut de la réponse non valide:', response.status);
          throw new Error(`Statut de la réponse non valide: ${response.status}`);
        }
